...
_is_able_to_do = switch (AIS_MEDICAL_EDUCATION) do {
	case (0) : {true};
	case (1) : {(items player) find "FirstAidKit" > -1 || {(items player) find "OPTRE_Biofoam" > -1} || {(items player) find "Medikit" > -1} || {(items player) find "OPTRE_Medigel" > -1} || {(items player) find "OPTRE_MedKit" > -1}};
